...
function generateReplacement(text) {
    ...
    section = text;
    while (text!=null){
    	queue.push (regex.exec(section));
    	section = section.slice(section.search(regex)+1,section.length);
    }
    if (section != null) { 
    	var edit = section;
    	//var end = section.charAt(length-1).toLowerCase();
    	return text.replace(regex, queue.shift()).concat(end));
	}
}
